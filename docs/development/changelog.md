...
### Added
- **Native TUI for imports** — "Import from Shelf" and "Import from Repo" now run entirely within the TUI instead of dropping to console; Import from Shelf reads another shelfctl shelf's catalog, deduplicates by SHA256, lets you multiselect books, downloads release assets and uploads to the destination shelf; Import from Repo scans any GitHub repo for book files (pdf, epub, mobi, etc.), lets you multiselect files, downloads/ingests/uploads them with auto-generated metadata, and records migrations in a JSONL ledger for resumability
