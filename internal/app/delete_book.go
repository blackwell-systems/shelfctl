...
// runDeleteFromUnified runs the delete workflow from within unified TUI mode
// This is called after the unified TUI has exited
func runDeleteFromUnified() error {
	// Collect all books for picker
	var allItems []tui.BookItem
	for i := range cfg.Shelves {
		shelf := &cfg.Shelves[i]
	...
	// Show multi-select book picker
	booksToDelete, err := tui.RunBookPickerMulti(allItems, "Select books to delete")
	if err != nil {
		return err
	}
	...
	if len(booksToDelete) > 0 {
		fmt.Println()
		fmt.Printf("You are about to delete %d books:\n", len(booksToDelete))
		for _, item := range booksToDelete {
			fmt.Printf("  - %s (%s)\n", item.Book.ID, item.Book.Title)
		}
		fmt.Println()
		fmt.Println("This will:")
		fmt.Println("  • Remove catalog entries")
		fmt.Println("  • Delete files from GitHub Release assets")
		fmt.Println("  • Clear from local cache")
		fmt.Println()
		fmt.Printf("Type 'DELETE %d BOOKS' to confirm: ", len(booksToDelete))
	...
	fmt.Println()
	if len(booksToDelete) == 1 {
		ok("Book deleted: %s", booksToDelete[0].Book.ID)
	} else {
		if successCount > 0 {
			ok("Successfully deleted %d books", successCount)
		}
