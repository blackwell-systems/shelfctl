...
// runEditFromUnified runs the edit workflow from within unified TUI mode
// This is called after the unified TUI has exited
func runEditFromUnified() error {
	...
	booksToEdit, err := tui.RunBookPickerMulti(allItems, "Select books to edit")
	if err != nil {
		return err
	}
	...
	successCount := 0
	failCount := 0
	allUpdatedBooks := make(map[string]catalog.Book)
	...
	if len(booksToEdit) == 1 {
		if successCount == 1 {
			ok("Book successfully updated: %s", booksToEdit[0].Book.ID)
		}
	} else {
		if successCount > 0 {
			ok("Successfully updated %d books", successCount)
