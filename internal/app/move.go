...
// runMoveFromUnified runs the move workflow from within unified TUI mode
// This is called after the unified TUI has exited
func runMoveFromUnified() error {
	// Run interactive move (book picker + destination picker)
	params := &moveParams{}
	...
	// Run interactive batch move
	if err := runInteractiveBatchMove(selected, params); err != nil {
		return err
	}
